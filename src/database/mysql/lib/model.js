...
module.exports = (function () {
	...
	var Row = Class({
			...
			async.waterfall([
				...
				function (res, cb) {
					if (res.changedRows === 0) {
						cb(new Error(Model.OPTIMISTIC_LOCK_EXCEPTION));
					} else {
						cb(null, res);
					}
				},
		...
		_load: function (conn, name, cb) {
			...
			cfg = this._table._lookupLinkedByMap(name);
			if (cfg) {
				return this.linkedBy(conn, name, cb);
			}

			cfg = this._table._lookupRelatesToMap(name);
			if (cfg) {
				return this.relatesTo(conn, name, cb);
			}

			cb(new Error('no linksTo or linkedBy or relatesTo: ' + name + ' defined on: ' + this._table.getName()));
		},
		...
		linksTo: function (conn, name, cb) {
			...
			if (cfg) {
				...
				cb(new Error('no linksTo: ' + name + ' defined on: ' + this._table.getName()));
			}
		},
		...
		linkedBy: function (conn, name, cb) {
			...
			cb(new Error('no linkedBy: ' + name + ' defined on: ' + this._table.getName()));
		},
		...
		relatesTo: function (conn, name, cb) {
			...
			cb(new Error('no relatesTo: ' + name + ' defined on: ' + this._table.getName()));
		},
	});
	...
	var Table = Class({
		...
		_create: function (conn, dto, callback) {
			var me = this;
			...
			async.waterfall([
				...
				function (res, _, next) {
					...
					next(null, new me._rowClass(dto));
				},
			], callback);
		},
